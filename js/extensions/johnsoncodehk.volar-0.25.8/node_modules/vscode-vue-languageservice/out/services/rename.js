...
function register({ mapper }) {
    ...
    function onTsPrepare(uri, position) {
        for (const tsRange of mapper.ts.to(uri, position)) {
            if (tsRange.data.capabilities.rename === true
                ...
                const tsPrepare = tsRange.languageService.prepareRename(tsRange.textDocument.uri, tsRange.range.start);
                if (!tsPrepare)
                    continue;
    ...
    function onCssPrepare(uri, position) {
        ...
        for (const cssRange of mapper.css.to(uri, position)) {
            ...
            const wordStart = shared_1.getWordStart(wordPattern, cssRange.range.end, cssRange.textDocument);
            if (wordStart) {
    ...
    function onCss(uri, position, newName) {
        ...
        // vue -> css
        for (const cssRange of mapper.css.to(uri, position)) {
            const cssWorkspaceEdit = cssRange.languageService.doRename(cssRange.textDocument, cssRange.range.start, newName, cssRange.stylesheet);
            if (cssWorkspaceEdit) {
                hasResult = true;
                    ...
                    for (const vueRange of mapper.ts.from(tsDocEdit.textDocument.uri, tsEdit.range.start, tsEdit.range.end)) {
                        if (isValidRange(vueRange.data)) {
                            _vueDocEdit.edits.push({
                                annotationId: node_5.AnnotatedTextEdit.is(tsEdit.range) ? tsEdit.range.annotationId : undefined,
