...
function register(languageService, getTextDocument) {
    ...
    function getIncomingCalls(item) {
        ...
        if (!document)
            return [];
        const fileName = shared_1.uriToFsPath(item.uri);
    ...
    function fromProtocolCallHierarchyItem(item) {
        ...
        const document = getTextDocument(shared_1.fsPathToUri(item.file)); // TODO
        ...
        const result = {
            ...
            name,
            detail,
            uri: shared_1.fsPathToUri(item.file),
            range: {
    ...
    function fromProtocolCallHierchyIncomingCall(item) {
        const document = getTextDocument(shared_1.fsPathToUri(item.from.file));
        return {
